...
const url_location = 'get';
let token = localStorage.getItem('jwt');//驗證jwt的token
var ta_json,serchyear,mylinechart;
var chart_btn = document.getElementById('draw-chart');
...
function getTAData() {
  ...
  .then(function (response) {
      ...
      var optionsCircle = {
        ...
        series: [ta_json['templist'], ta_json['humlist']*100,30,15],
        labels: ['當前溫度', '當前濕度','當前冰水出水','當前冷卻水回水'],
        legend: {
          show: true,
          position: 'left',
          offsetX: -30,
          ...
          formatter: function (val, opts) {
            return val + " - " + opts.w.globals.series[opts.seriesIndex] + '度'
          }
        },
        fill: {
          type: 'gradient',
      ...
      window.setInterval(function () {
       ...
       // chartCircle.updateSeries([getRangeRandom({ min: 10, max: 100 }), getRangeRandom({ min: 10, max: 100 })])
        ...
        var p1Data = getRangeRandom({ min: 10, max: 30 });
        ...
        var p2Data = getRangeRandom({ min: 10, max: 30 });
        ...
        var p3Data = getRangeRandom({ min: 10, max: 30 });
